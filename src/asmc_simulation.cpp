...
void odom_callback(const nav_msgs::Odometry::ConstPtr& odom)
{
  ...
  tf2::Quaternion myQuaternion(q0,q1,q2,q3);
  tf2::Matrix3x3 m(myQuaternion);
  m.getRPY(theta, phi, psi_q);
  psi = (float)(psi_q);
...
int main(int argc, char *argv[])
{
  ...
  ros::init(argc, argv, "asmc_simulation");
  ...
  float miu_u = 0.05;
  float miu_psi = 0.1;
  float lambda_u = 0.001;
  float lambda_psi = 1;
  
  n.getParam("/asmc_simulate/k_u", k_u);
  n.getParam("/asmc_simulate/k_psi", k_psi);
  n.getParam("/asmc_simulate/kmin_u", kmin_u);
  n.getParam("/asmc_simulate/kmin_psi", kmin_psi);
  n.getParam("/asmc_simulate/k2_u", k2_u);
  n.getParam("/asmc_simulate/k2_psi", k2_psi);
  n.getParam("/asmc_simulate/mu_u", miu_u);
  n.getParam("/asmc_simulate/mu_psi", miu_psi);
  n.getParam("/asmc_simulate/lambda_u", lambda_u);
  n.getParam("/asmc_simulate/lambda_psi", lambda_psi);
